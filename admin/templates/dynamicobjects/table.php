...
<?php $i = 0; foreach ($dynamicobjects as $objId => $obj): $i++; ?>
	<tr class="<?php echo $i % 2 == 0 ? "background-blue-light" : "" ?>">
		...
		<td class="no-phone border-bottom border-mute-light text-ellipsis"><?php echo Configuration::getDynamicObject($objId)->getContent() ?></td>
		<td class="border-right border-bottom border-mute-light text-right"><a href="../<?php echo $obj['Page'] . "#" . $obj['ObjectId'] ?>" target="_blank" class="fa icon-large fa-search fore-mute"></a></td>
	</tr>
<?php endforeach; ?>
<?php endif; ?>
</table>
